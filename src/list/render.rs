...
/// Trait for items that can be displayed in a list pane.
trait ListItem {
    fn display_name(&self) -> &str;
}

impl ListItem for InstalledSkill {
    fn display_name(&self) -> &str {
        &self.name
    }
}

impl ListItem for AssetEntry {
    fn display_name(&self) -> &str {
        &self.name
    }
}
